...
class JobsDiagnose extends Command
{
    ...
    public function handle(JobTransport $transport): int
    {
        ...
        $media = config('job_transport.media_queue');
        $connected = false;
        $worker = null;
        $rows = [];
        ...
        try {
            ...
            foreach ([$generation, $deployment, $media] as $queue) {
                ...
                $rows[] = ["oldest $queue", $envelope ? (($envelope['created_at'] ?? 'unknown').' ('.($envelope['resource_id'] ?? 'invalid').')') : 'none'];
            }
        } catch (Throwable $error) {
            $rows[] = ['Redis error', $error->getMessage()];
        }

        $expected = ['database' => (int) env('REDIS_QUEUE_DB', env('REDIS_DB', 0)), 'generation_queue' => $generation, 'deployment_queue' => $deployment, 'media_queue' => $media, 'prefix' => config('job_transport.prefix')];
        ...
        $orphaned = GenerationRun::where('status', 'queued')->whereNull('started_at')->count()
            + Deployment::where('status', 'queued')->whereNull('started_at')->count();
        $rows[] = ['orphaned queued records', (string) $orphaned];
