...
class GenerationController extends Controller
{
    ...
    {
        $run = $project->generationRuns()->create(['provider' => $request->validated('provider', env('AI_PROVIDER', 'mock')), 'status' => 'queued', 'progress' => 0, 'input' => $request->validated('input'), 'queued_at' => now()]);
    ...
    public function retry(GenerationRun $generationRun): JsonResponse
    {
        ...
        $retry = $generationRun->project->generationRuns()->create(['provider' => $generationRun->provider, 'status' => 'queued', 'progress' => 0, 'input' => $generationRun->input, 'queued_at' => now(), 'attempt' => $generationRun->attempt + 1, 'max_attempts' => $generationRun->max_attempts]);
