...
class DashboardOverviewController extends Controller
{
    public function __invoke(Request $request, EntitlementService $entitlements, UsageService $usage): JsonResponse
    {
        ...
        return response()->json(['data' => [
            ...
            'recent_deployments' => $deployments->map(fn ($item) => ['id' => $item->id, 'project_id' => $item->project_id, 'project_name' => $item->project_name, 'status' => $item->status, 'dry_run' => $item->dry_run, 'completed_at' => $item->completed_at, 'site_url' => $item->connection?->site_url]),
