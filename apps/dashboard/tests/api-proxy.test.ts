...
test('buffers JSON, preserves status, and consumes the upstream body once', async () => {
  ...
  const stream = new ReadableStream({
    start(controller) {
      controller.enqueue(
        new TextEncoder().encode('{"data":[{"id":1},{"id":2}]}'),
      );
      controller.close();
    },
    pull() {
...
test('supports an empty project list and upstream error responses', async () => {
  const empty = await bufferedProxyResponse(
    Response.json({ data: [] }),
    String,
  );
  assert.deepEqual(await empty.json(), { data: [] });
  const failure = await bufferedProxyResponse(
    Response.json({ error: { message: 'failed' } }, { status: 500 }),
    String,
  );
...
test('an aborted projects fetch becomes a controlled timeout error', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof DashboardApiError && error.status === 504,
  );
});
