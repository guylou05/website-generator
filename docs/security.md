...
Stripe webhook signatures are verified before processing. Billing is organization-scoped and owner/admin-only; redirect origins and server-owned price mappings are allowlisted. Secret keys, card data, full webhook payloads, and hosted session URLs are excluded from browser state and audit logs.
