...
const blueprint = JSON.parse(await readFile(new URL('../../shared/sample-blueprint.json', import.meta.url)));
const snapshot = {
  capturedAt: '2026-08-02T00:00:00Z', pages: [], media: [], menus: [],
  homepage: { showOnFront: 'posts', pageId: 0 }, settings: {},
...
test('creates a comprehensive, immutable dry-run plan without a WordPress client', () => {
  const plan = createDeploymentPlan({ blueprint, snapshot, elementorPages: { [blueprint.pages[0].id]: [{ id: 'hero' }] }, setHomepage: true });
  assert.equal(plan.readOnly, true);
  assert.equal(plan.safetyStatus, 'safe');
  assert.ok(plan.changes.some((change) => change.resource === 'page' && change.action === 'create'));
...
test('blocks a plan when Elementor changes cannot be applied safely', () => {
  const plan = createDeploymentPlan({ blueprint, snapshot: { ...snapshot, elementor: { active: false, cssCachePresent: false } }, elementorPages: { [blueprint.pages[0].id]: [] } });
