...
export const prompts = {
  ...
  writing: `Write clear, specific, conversion-focused page copy. Lead with customer outcomes, explain each service plainly, answer objections in FAQs, and use honest calls to action. Do not use hype or unverifiable claims. Return pages and sections as arrays; put each stable page or section identifier in its key field.`,
  seo: `Create a local SEO strategy for every planned page. Use natural service-and-location intent when location is known, unique titles and descriptions, and focused keywords without stuffing. Return pages as an array with each stable page identifier in its key field.`,
  design: `Select an accessible, professional design direction. Use valid hex colors, readable contrast, broadly available font families, and semantic layout choices rather than CSS or page-builder settings. Return pageLayouts and their sections as arrays with stable identifiers in key fields.`,
